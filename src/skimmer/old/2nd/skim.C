...
void Skim::go()
{
  ...
  uint16 * histogram = new uint16[4000];
  memset(histogram,0,8000);
  uint16 * midhist=histogram+2000;
  ...
  for(uint32 readnr=0; readnr<numseq; readnr++){
    ...
    for(int16 readpos=0; readpos< SKIM_hashperseq[readnr];readpos++, hashp++, hashokp++){
      ...
      for(uint32 j=0; j<SKIM_forwardhashcount[*hashp]; j++, rahpsph++) {
	if(rahpsph->read<=readnr) continue;  
	count[rahpsph->read]++;
...
void Skim::transformSeqToHash (const char * seq, uint32 slen, const char * readname, uint16 * hashp, uint8 * hashokp, uint32 * hashcount)
{
  ...
  for(int32 i=0; i<slen; i++, seq++){
    ...
    case '*' : {
      ...
      // uncomment this to break hash making
      //	acthash=0;
      //	baseok=0;
	break;
    }
