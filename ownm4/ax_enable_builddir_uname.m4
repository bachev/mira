...
dnl   $3 = -all
dnl
dnl This macro must be called before AM_INIT_AUTOMAKE. It creates a
dnl default toplevel srcdir Makefile from the information found in the
dnl created toplevel builddir Makefile. It just copies the variables
dnl and rule-targets, each extended with a default rule-execution that
dnl recurses into the build directory of the current "BUILD". You can
dnl override the auto-dection through `uname -msr | tr " /" "__"`.d at
dnl build-time of course, as in
dnl
