...
dnl file library.pc if it exists in the pkgconfig data directory. The
dnl above information still needs to be specified, as it will still be
dnl used if pkg-config is not found on the host system at configure
dnl time.
dnl
dnl -I and -L flags will be obtained from *both* pkgconfig and the
dnl script. AC_CONFIG_LIBCONFIG_IN_PKGCONFIG enables pkg-config support
dnl (default) AC_CONFIG_LIBCONFIG_IN_STATIC disables pkg-config support
dnl
dnl There is also an AC_SUBST(LIBRARY_CONFIG) that will be set to the
dnl name of the file that we output in this macro. Use as:
dnl
